...
def search():
    ...
    force_words_dict = set(d.get('force_words', []))
    ...
    def solve(path, current_total_len):
        ...
        if len(path) == max_len:
            if not force_words_dict.issubset(set(path)): return
        ...
        targets = {shift_kana(src_char, abs(ks_val)), shift_kana(src_char, -abs(ks_val))} if use_shift and ks_val != 0 else {src_char}
        all_targets = set()
