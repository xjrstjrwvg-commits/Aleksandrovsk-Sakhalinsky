...
def search():
    ...
    # 餡式接続モードの取得
    is_an_mode = d.get('an_shiki_mode', False)
    ...
    def solve(path, current_total_len):
        ...
        if is_an_mode:
            # 餡式接続（3文字以上必須、語尾無視）
            last_word = path[-1]
            if len(last_word) < 3: return
            an_content = last_word[1:-1].replace("ー", "")
            an_chars = [get_base_char(c, filt_s, filt_d, filt_h) for c in an_content]
            ...
            for nxt in word_pool:
                ...
                if all(char in nxt_norm for char in an_chars):
