...
def search():
    ...
    if d.get('exclude_conjugate'):
        ...
        for w in temp_pool:
            ch, ct = get_clean_char(w, "head", 0, conn_s, conn_d, conn_h), get_clean_char(w, "tail", 0, conn_s, conn_d, conn_h)
            pair_map[f"{ch}_{ct}"].append(w)
        word_pool = [words[0] for words in pair_map.values() if len(words) == 1]
    else: word_pool = temp_pool
    ...
    def solve(path, current_total_len):
        ...
        if len_mode == 'diff' and len(path) > 1:
            ls = [len(x) for x in path]
            if len(ls) != len(set(ls)): return
        if len(path) == max_len:
            ...
            def check_l(lst):
                for grp in lst:
                    t_cnt, g_sh, items = 1, 0, []
                    for itm in grp:
                        if ':' in itm:
                            ps = itm.split(':')
                            if ps[0].upper() == 'S': g_sh = int(ps[1])
                            else: t_cnt = int(ps[1] if ps[1].isdigit() else ps[1])
                        else: items.append(itm)
                    tot = sum(norm_t.count("".join([get_base_char(shift_kana(c, g_sh), filt_s, filt_d, filt_h) for c in it])) for it in items)
                    if (d.get('exclusive_choice') and tot != t_cnt) or (not d.get('exclusive_choice') and tot < t_cnt): return False
                return True
            if not (check_l(d.get('group_constraints', [])) and check_l(d.get('choice_constraints', []))): return
        ...
        is_odd = (len(path) % 2 != 0)
        last_w_c = path[-1].replace("ー","")
        base_offs = [p_shift] + ([i for i in range(p_shift+1, len(last_w_c))] if d.get('auto_recovery') else [])
        
        for off in base_offs:
            ...
            for tc in targets:
                ...
                for nxt in cands:
                    ...
                    if d.get('ansiki_mode'):
                        ...
                        mid_chars = [get_base_char(c, filt_s, filt_d, filt_h) for c in prev_clean[1:-1]]
                        nxt_norm = "".join([get_base_char(c, filt_s, filt_d, filt_h) for c in nxt])
                        if not all(nxt_norm.count(mc) >= 1 for mc in mid_chars): continue
